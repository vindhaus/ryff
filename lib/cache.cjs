...
async function getStoreSafe() {
  ...
  try {
    ...
    // First try auto-config (works when Blobs is enabled on the site)
    try {
      _store = getStore(process.env.BLOBS_STORE || 'ryff-cache');
      return _store;
    } catch (err) {
      // If Blobs isn't configured in the runtime, try manual credentials
      if (err && err.name === 'MissingBlobsEnvironmentError') {
        ...
        if (siteID && token) {
          _store = getStore(process.env.BLOBS_STORE || 'ryff-cache', { siteID, token });
          return _store;
        }
        // give up → filesystem fallback
        _store = null;
        return null;
      }
      throw err;
